...
@section('content')
    <main class="flex-grow w-full pb-20">
        ...
        <div class="max-w-4xl mx-auto px-4 pt-12 pb-10">
            @if ($post->category)
                <div class="flex flex-wrap items-center gap-3 mb-6">
                    <a href="{{ route('category.show', $post->category->slug) }}"
                        class="text-xs font-bold tracking-widest text-orange-600 uppercase bg-orange-50 hover:bg-orange-100 transition-colors px-3 py-1 rounded-full">
                        {{ $post->category->name }}
                    </a>
                    @if ($post->tags && $post->tags->count() > 0)
                        @foreach ($post->tags as $tag)
                            <span
                                class="text-xs font-semibold tracking-wider text-gray-500 uppercase bg-gray-100 px-3 py-1 rounded-full">
                                #{{ $tag->name }}
                            </span>
                        @endforeach
                    @endif
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold leading-tight text-gray-900 mb-8">{{ $post->title }}</h1>

            <div class="flex items-center gap-4 text-gray-500">
                <div
                    class="w-12 h-12 rounded-full bg-orange-500 flex items-center justify-center text-lg text-white font-bold shadow-sm">
                    {{ substr($post->user->name, 0, 1) }}
                </div>
                <div>
                    <p class="font-semibold text-gray-900 text-lg">{{ $post->user->name }}</p>
                    <p class="text-sm">{{ $post->created_at->format('M d, Y') }} &middot;
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <article
                class="prose md:prose-lg max-w-none text-gray-700 prose-headings:text-gray-900 prose-a:text-orange-600 hover:prose-a:text-orange-500">
                {!! $post->content !!}
            </article>
            ...
            @if ($relatedPosts && $relatedPosts->count() > 0)
                ...
                <div class="mt-16 pt-12 border-t border-gray-100">
                    <h3 class="text-2xl font-bold mb-6 text-gray-900">Berita Terkait</h3>
                        ...
                        @foreach ($relatedPosts as $related)
                            <a href="{{ route('posts.show', $related->slug) }}"
                                ...
                                <div class="absolute bottom-0 left-0 right-0 p-4">
                                    <h4
                                        class="text-white font-bold leading-tight group-hover:text-orange-400 transition-colors">
            ...
            <div class="mt-16 pt-8 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}"
                        target="_blank"
                        class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-orange-50 hover:text-orange-600 transition-colors"
                    ...
                    <a href="https://api.whatsapp.com/send?text={{ urlencode($post->title . ' ' . request()->fullUrl()) }}"
                        ...
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M12.031 0c-6.627 0-12 5.373-12 12 0 2.19.596 4.252 1.636 6.021l-1.667 6.084 6.223-1.632c1.725.952 3.708 1.496 5.808 1.496 6.627 0 12-5.373 12-12s-5.373-12-12-12zm0 21.896c-1.874 0-3.625-.487-5.15-1.353l-.37-.21-3.816 1.001 1.018-3.717-.23-.365c-.958-1.534-1.472-3.32-1.472-5.19 0-5.467 4.449-9.916 9.916-9.916s9.916 4.449 9.916 9.916-4.449 9.916-9.916 9.916zm5.45-7.443c-.299-.15-1.767-.872-2.04-.972-.272-.1-.471-.15-.67.15-.199.3-.771.972-.945 1.171-.174.2-.348.225-.647.075-.299-.15-1.26-.465-2.4-1.485-.886-.795-1.484-1.777-1.658-2.077-.174-.3-.018-.462.132-.612.135-.135.299-.35.449-.525.15-.175.2-.299.299-.499.1-.2.05-.375-.025-.525-.075-.15-.67-1.615-.918-2.21-.242-.579-.487-.5-.67-.51-.174-.01-.373-.01-.572-.01-.199 0-.522.075-.796.375-.274.3-1.045 1.02-1.045 2.485 0 1.465 1.07 2.88 1.219 3.08.15.2 2.095 3.195 5.076 4.482.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.086 1.767-.722 2.016-1.422.249-.7.249-1.295.174-1.422-.074-.127-.274-.2-.573-.35z" />
                        </svg>
                    </a>
                </div>

                <a href="{{ route('home') }}"
                    class="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 transition-colors font-medium mt-6 sm:mt-0">
